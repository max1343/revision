...
void depth_cb(freenect_device *dev, void *v_depth, uint32_t timestamp)
{
	...
	uint8_t* profProche = (uint8_t*)malloc(640*480*3);
	int cpt = 0, cpm = 0;

	for (i=0; i<640*480; i++) {
		if(depth_mid[3*i+0]>=150 ){//}&& depth_mid[3*i+1]<=23 && depth_mid[3*i+2]<=23){
			//c'est tout noir => objet proche
			profProche[3*i+0] = 1;
			profProche[3*i+1] = 1;
			profProche[3*i+2] = 1;
			cpm+=i;
